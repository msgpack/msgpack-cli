...
namespace MsgPack.Serialization.DefaultSerializers
{
	...
	internal sealed class FSharpMapSerializer<T, TKey, TValue> : MessagePackSerializer<T>
		where T : IEnumerable<KeyValuePair<TKey, TValue>>
	{
		private readonly Func<IEnumerable<Tuple<TKey, TValue>>, T> _factory;

		private static Func<IEnumerable<Tuple<TKey, TValue>>, T> FindFactory()
		{
			var factoryType =
					...
					_ =>
					{
						throw new NotSupportedException(
							String.Format(
								...
								"Cannot find MapModule. '{0}' may not be an fsharp collection.",
...
#if !UNITY
			return result.CreateDelegate( typeof( Func<IEnumerable<Tuple<TKey, TValue>>, T> ) ) as Func<IEnumerable<Tuple<TKey, TValue>>, T>;
#else
			return Delegate.CreateDelegate( typeof( Func<IEnumerable<Tuple<TKey, TValue>>, T> ), result ) as Func<IEnumerable<Tuple<TKey, TValue>>, T>;
#endif // !UNITY
		}
		...
		public FSharpMapSerializer( SerializationContext ownerContext, PolymorphismSchema keysSchema, PolymorphismSchema valuesSchema )
		...
		protected internal override T UnpackFromCore( Unpacker unpacker )
		{
			...
			var buffer = new Tuple<TKey, TValue>[ UnpackHelpers.GetItemsCount( unpacker ) ];
			...
			using ( var subTreeUnpacker = unpacker.ReadSubtree() )
			{
				for ( int i = 0; i < buffer.Length; i++ )
				{
					...
					buffer[ i ] = new Tuple<TKey, TValue>( key, value );
				}
			}
		...
		protected internal override void UnpackToCore( Unpacker unpacker, T collection )
		{
			throw new NotSupportedException(
				String.Format(
					CultureInfo.CurrentCulture,
					"Unable to unpack items to existing F# map '{0}'.",
					typeof( T )
				)
			);
		}
