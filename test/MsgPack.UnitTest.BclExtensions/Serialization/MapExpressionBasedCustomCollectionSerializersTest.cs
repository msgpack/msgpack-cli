...
{
	[TestFixture]
	public class MapExpressionBasedImmutableCollectionsTest
	{
		private MessagePackSerializer<T> CreateTarget<T>()
		{
			var context = new SerializationContext() { SerializationMethod = SerializationMethod.Map, EmitterFlavor = EmitterFlavor.ExpressionBased };
			return ( new ExpressionTreeSerializerBuilder<T>() ).BuildSerializerInstance( context, typeof( T ), PolymorphismSchema.Default );
		}
